...
{
    $handles = array('wordfenceAJAXcss', 'dashicons', 'yoast-seo-adminbar', 'admin-bar', 'wp-block-library', 'frm_fonts', 'plugins-css', 'main-css', 'formidable', 'wc-blocks-vendors-style', 'wc-blocks-style', 'woocommerce-layout', 'woocommerce-smallscreen', 'woocommerce-general');
    ...
    ob_start();
     ...
     if (!empty($cart)) {
         foreach ( $cart as $cart_item_key => $cart_item ) {
            $fields = get_fields($cart_item['product_id']);
            break;
        }

        if ($fields['free_bonus_items'] && $qty > 0) {

        ?>
            <div class="bonus-items">
                <?php foreach ( $fields['free_bonus_items'] as $key => $val ) { ?>
                    <div class="item">
                        ...
                        <h4><?= $val['name'] ?></h4>
                        <div class="prices ms-auto">
                            <span class="default">$<?= number_format( ($val['price'] * $qty), 2) ?></span>
                            <span>FREE</span>
                        </div>
                    </div>
                <?php } ?>
     ...
     $fragments['.bonus-items'] = ob_get_clean();
